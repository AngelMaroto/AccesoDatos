...
public class UserRestController {
    ...
    /*
    La inyección de dependencias es un patrón de diseño utilizado
    en el desarrollo del software, ayudándonos a potenciar la modularidad,
    flexibilidad y mantenimiento de nuestro código (usado en los lenguajes orientado
     a objeto).
     */

    /*
    Básicamente porque esta anotación especifica que se inyectará un objeto de spring
    (bean o componente) en un atributo de otro objeto.
    En otras palabras, nos proporciona control cuando inyectamos nuestras dependencias
     */
    @Autowired
    ...
    public User getUserEmail(@PathVariable String userEmail){
        ...
        //retornará al usuario con email pasado en la url
        return user;
    }
    ...
    public String deteteUserEmail(@PathVariable String userEmail) {

        User user = userService.findByEmail(userEmail);

        if(user == null) {
            throw new RuntimeException("User id not found -"+userEmail);
        }

        userService.deleteByEmail(userEmail);

        //Este método, recibira el email de un usuario en la url y lo borra de la base de datos
        return "Deleted user email - "+userEmail;
    }
}
